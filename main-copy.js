    ...
    if (totalItems > 0) {
        ...
        async function processPerson(person, newPage) {
            ...
            try {
                ...
                // If more than 1 email is found, populate Email 2
                if (emails.length > 1) {
                    person.email2 = emails[1];
                }
            } catch (error) {
                console.error(`Error processing ${person.fullName} at ${cleanedUrl}: ${error}`);
                person.emails = [''];
            }
        }
        ...
        for (let i = 0; i < allData.length; i += batchSize) {
            ...
            await Promise.all(batch.map(async person => {
                ...
                try {
                    ...
                    console.error(`Error processing ${person.fullName}: ${error}`);
                } finally {
                    await newPage.close();
                }
            }));
        ...
        const csvHeader = `First Name,Last Name,Full Name,Job Title,Company Name,Location,Employee Count,Phone,Industry,URL,LinkedIn URL,Facebook URL,Twitter URL,Company LinkedIn URL,Company URL,Keywords,${emailHeaders},Email 2\n`;

        const csvRows = allData.map(person => {
            const paddedEmails = [...person.emails, ...Array(maxEmails - person.emails.length).fill('')];
            return `${person.firstName},${person.lastName},${person.fullName},${person.jobTitle},${person.companyName},${person.location},${person.employeeCount},${person.phone},${person.industry},${person.firstHref},${person.linkedinUrl},${person.facebookUrl},${person.twitterUrl},${person.companyLinkedinUrl},${person.companyUrl},${person.keywords},${paddedEmails.join(',')},${person.email2 || ''}`;
        }).join('\n');
